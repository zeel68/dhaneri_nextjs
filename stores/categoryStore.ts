  ...
  persist(
    (set, get) => ({
      ...
      createCategory: async (categoryData: any) => {
        set({ loading: true, error: null })
        try {
          const response = (await apiClient.post("/store-admin/category", categoryData)) as any
      ...
      updateCategory: async (id: string, categoryData: any) => {
        set({ loading: true, error: null })
        try {
          const response = (await apiClient.put(`/store-admin/category/${id}`, categoryData)) as any
          console.log(response)
          ...
          if (response.success) {
            ...
            set({
              categories: state.categories.map((c: any) => (c._id === id ? { ...c, ...updatedCategory } : c)),
              loading: false,
            })
      ...
      deleteCategory: async (id: string) => {
        set({ loading: true, error: null })
        try {
          const response = (await apiClient.delete(`/store-admin/category/${id}`)) as any
      ...
      toggleCategoryStatus: async (id: string, status: boolean) => {
        set({ loading: true, error: null })
        try {
          const response = (await apiClient.post(`/store-admin/category/${id}/status`, { status })) as any
